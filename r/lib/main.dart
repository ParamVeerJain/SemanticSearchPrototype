...
class _ProductSearchScreenState extends State<ProductSearchScreen> {
  ...
  Future<void> _searchProducts(String query) async {
    ...
    final response = await http.post(
      Uri.parse('http://localhost:8888/text_search'),
    ...
    if (response.statusCode == 200) {
      ...
      print(
          'Failed to load products: ${response.statusCode}'); // Debugging line
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _uploadImage() async {
    ...
    final url = Uri.parse('http://localhost:8888/image-search');
    final response = await http.post(
      url,
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                          ...
                          color: Colors.grey[300],
                          child: Image.network(
                            'http://localhost:8888/images/${product.id}',
                            fit: BoxFit.cover,
                          ),
                        ),
                        const SizedBox(width: 10),
...
class Product {
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    try {
      ...
      return Product(
        id: json['productId'] ?? 0,
        displayNames: json['productDisplayName'] ?? 'Unknown Product',
        descriptions: json['description'] ?? 'No description available',
        averageRatings: (json['averageRating'] ?? 0).toDouble(),
